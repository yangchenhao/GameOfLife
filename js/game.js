...
function createTable(map) {
    ...
    for (var i = 0; i < TableSize; i++) {
            ...
            for (var j = 0; j < TableSize; j++) {
               ...
               cell.height=""+600/TableSize+"px";
               cell.width=""+600/TableSize+"px";
...
function refresh(){
    if(!pause){
        t=setTimeout(function(){grow();refresh();},RefreshTime);
    }
    else{
        Pause();
        return;
...
function createNewMap(){
    for(var i=0;i<TableSize;i++){
        for (var j=0;j<TableSize;j++){
            var live = livePoint(i,j);  
            if(live == 3){
                newmap[i][j] = true;
            }
            else if(live == 2 && map[i][j] == true){
                newmap[i][j] = true;
            }
            else{
                newmap[i][j] = false;
            }
            pause = pause&&(map[i][j]==newmap[i][j]);
        }
    }
...
function refreshMap(map){
    for(var i=0; i<TableSize; i++){
        for(var j=0; j<TableSize; j++){
            if(map[i][j]){document.getElementById('table1').childNodes.item(2).rows[i].cells[j].bgColor="Red";}
            else {document.getElementById('table1').childNodes.item(2).rows[i].cells[j].bgColor="White";}
        }
    }
}
